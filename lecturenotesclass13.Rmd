...
```{r data_manhattan}
manhattan <- read_csv("~/R/lecture-note-class-13-mayapandey130/manhattan.csv")
```
...
The 95% bootstrap confidence interval for the population medain price of a 1-bedroom apartment in Manhattan is between 2162 and 2875. 
...
We are 95% confident that the total proportion of mice that died while not on the treatment is between 0.690 and 0.797
...
```{r practice_2_ci}
ci_mice_lived <- get_ci(boot_prop_live, level = .9)
ci_mice_lived
```
...
We are 90% confident that the total proportion of mice that lived while on the treatment is between 0.552 and 0.651. This can be seen in the graph in the blue shaded region. 
